...
Users choose **Local LLM** or **Cloud API** under **Settings > AI drafting**. A workspace administrator must enable OpenAI or Gemini first. Every cloud draft and selected-paragraph regeneration displays a confirmation naming the provider and the official context that will leave the workspace.
