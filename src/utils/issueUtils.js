...
export function normalizeIssue(input = {}) {
  ...
  return {
    ...
    owningDivisionId: input.owningDivisionId || '',
    visibility: ['workspace', 'division', 'restricted'].includes(input.visibility) ? input.visibility : 'workspace',
    accessLevel: ['editor', 'viewer'].includes(input.accessLevel) ? input.accessLevel : '',
