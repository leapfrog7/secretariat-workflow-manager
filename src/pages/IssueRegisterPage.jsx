...
import { Activity, AlertTriangle, CalendarClock, CircleDashed, ClipboardList, MessageCircleQuestion } from 'lucide-react';
...
import { bringBackIssue, getAllIssues, restoreIssue } from '../db/issueRepository';
  ...
  return (
    <>
      ...
      <div className="space-y-4">
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5" aria-label="Issue summary">
        ...
        <ArchiveViewSwitch
          ...
          onChange={(archiveMode) => setFilters((current) => ({ ...current, archiveMode, sort: archiveMode === 'Scheduled' ? 'Next appearance' : current.sort === 'Next appearance' ? 'Recently updated' : current.sort }))}
        />

        <section className="surface rounded-md p-3 sm:p-4">
          <div className="grid gap-3 lg:grid-cols-[minmax(240px,1fr)_minmax(440px,auto)] lg:items-end">
            ...
            <FilterBar filters={filters} onChange={setFilters} onClear={() => setFilters((current) => ({ ...defaultFilters, archiveMode: current.archiveMode, sort: current.archiveMode === 'Scheduled' ? 'Next appearance' : defaultFilters.sort }))} />
          </div>
        </section>
        ...
        {!filtered.length ? (
          ...
          <>
            <IssueTable issues={filtered} officers={data.officers} registerMode={filters.archiveMode} workingId={workingId} onRestore={restore} onBringBack={bringBack} />
            <div className="space-y-3 md:hidden">
              {filtered.map((issue) => (
                <IssueCard key={issue.id} issue={issue} officers={data.officers} registerMode={filters.archiveMode} working={workingId === issue.id} onRestore={restore} onBringBack={bringBack} />
              ))}
            </div>
          </>
        )}
...
function Metric({ label, value, detail, icon: Icon, tone }) {
  return (
    <div className={`surface min-h-28 rounded-md border-t-4 p-4 ${metricTones[tone]}`}>
...
function ArchiveViewSwitch({ value, currentCount, scheduledCount, archivedCount, onChange }) {
  const options = [
    ...
    { label: 'All', count: currentCount + scheduledCount + archivedCount },
  ];
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h2 className="text-sm font-semibold text-[#17333b]">Register view</h2>
        <p className="mt-0.5 text-xs text-slate-500">Current work, scheduled returns and permanent archive.</p>
      </div>
      <div className="inline-flex max-w-full overflow-x-auto rounded-md border border-[#cfdedb] bg-white p-1 shadow-sm" role="group" aria-label="Register view">
        {options.map((option) => (
