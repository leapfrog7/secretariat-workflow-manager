      ...
      <div className="grid grid-cols-[minmax(0,1fr)] items-start gap-5 lg:grid-cols-[220px_minmax(0,1fr)]">
        ...
        <div className="min-w-0 space-y-5" style={{ overflowWrap: 'anywhere' }}>
          ...
          <HelpSection id="access" icon={Users} title="Account and workspace access" tone="blue">
            ...
            <GuideRows rows={[
              ['Create an account', 'Register with your email address and choose your own password. A new account remains pending and cannot open official work.'],
              ['Approval and placement', 'The System Administrator selects your primary workspace and starting role when approving the account. Both become active together.'],
          ...
          <HelpSection id="workspace" icon={FileText} title="Inside an Issue" tone="amber">
            ...
            <div className="divide-y divide-slate-200 border-y border-slate-200">
              <WorkspaceRow icon={History} title="Current Position">Use the edit icon beside Present position in the Issue register for a quick update without opening the full Issue. Add update preserves the earlier position as a milestone; Correct latest is only for fixing the currently displayed entry. The full Current Position tab also manages stage, assignment and position history.</WorkspaceRow>
              <WorkspaceRow icon={FileText} title="Running Summary">Maintain one concise note containing the facts, decisions and pending work. Use Preview to check headings, lists or tables. Paste longer notings and select Summarize with AI to prepare a shorter version for review before saving.</WorkspaceRow>
              ...
              <WorkspaceRow icon={FileText} title="References">Create rules, O.M.s, instructions and other authorities once in the shared Reference Library. Paste text or import Word, text or selected PDF pages; scanned pages support English, Hindi and bilingual OCR. Save useful passages as extracts. In an Issue, attach the library entry and record why it matters or which extracts should enter AI context. Detaching it from one Issue does not delete the shared authority.</WorkspaceRow>
              <WorkspaceRow icon={FileText} title="Casework: Examine and Note">Open Casework from the main navigation and search for an Issue by title, eFile number or present position. From an open Issue, use Open Casework in the page header. Recent returns to saved Notes and Drafts; Awaiting lists matters awaiting input or discussion. In large cloud workspaces search is paged on the server and automatically falls back to the synchronized local copy when offline. Add Note opens a focused editor. Help me write uses four guided steps: choose the Note setup; define its objective and analytical emphasis; select the running summary, previous Notes, communications, references or temporary documents; then review the complete instruction and provider before preparing. Selectable PDF text is extracted directly, while scanned pages can be read locally with English, Hindi or bilingual OCR. Files and pasted text remain temporary and are not stored with the Issue. After a Note exists, Refine with AI follows one loop: describe the change, review the revision in the editor, then Accept changes or Restore original. Preview in editor can be turned off. Revision selection, the conversation and paragraph-level differences are grouped under History and change details. All AI wording remains unsaved until Save note is selected. Download Word creates an editable DOCX copy for further work outside the application. Notes remain editable after discussion, while earlier saved wording stays in history.</WorkspaceRow>
              <WorkspaceRow icon={Bot} title="Casework: Prepare Communication">Select Prepare communication on a Note to carry that Note, its linked records, the running summary and current position into Drafting. Start blank draft opens the editor immediately. Prepare with AI uses four guided steps: choose the format, signatory and recipient; define the outcome and document details; select Notes, communications, references and Issue context; then review the full brief, provider, warnings and prompt before preparing. In the editor, select body text before using Rewrite selection; use More and Prepare again with AI only when a new full draft is required.</WorkspaceRow>
          ...
          <HelpSection id="settings" icon={Settings} title="Using Settings" tone="blue">
            ...
            <GuideRows rows={[
              ...
              ['Office profile', 'Enter the Ministry, Department, office address and other heading details used in generated communication. Select authorized signatories and save the default Word font, size, line spacing, paragraph spacing and margins.'],
          ...
          <HelpSection id="ai" icon={Bot} title="Preparing an AI-assisted draft" tone="violet">
            <GuideLocation link="/casework" label="Open Casework">Casework &gt; choose an Issue &gt; Prepare Communication</GuideLocation>
            <ol className="space-y-3">
              <Instruction number="1" title="Prepare the office profile">In Settings, enter the Ministry or Department, bilingual heading if required, place of issue and house-style notes. Select authorized signatories and the default document style.</Instruction>
              ...
              <Instruction number="4" title="Review information used">Preparing a communication from a Note automatically includes that Note, its linked records, the running summary and current position. Open Information used only when you need to add or remove material.</Instruction>
              <Instruction number="5" title="Complete details while drafting">Open Draft tools beside the editor on desktop, or use its floating button on mobile. The Details tab lets you add or correct the communication number, date, subject, addressee, communication type and authorised signatory without regenerating the body.</Instruction>
              <Instruction number="6" title="Choose or change the format">Choose Letter, Office Memorandum, Order, Office Order or D.O. Letter from the template cards; More reveals specialized formats such as Notification, Endorsement and Press Communique. Hover or focus a card to choose Start blank or Prepare with AI. On touch devices both actions remain visible. Prepare with AI guides you through setup, objective, sources and final review. Advanced settings on the Review step lets you add your own drafting direction and inspect the exact prompt and approximate token size before sending it; protected factual and formatting rules remain read-only. In an open draft, change the format through the Details side tool; the substantive body is preserved.</Instruction>
              <Instruction number="7" title="Reuse wording and addresses">Switch Draft tools to Paragraph Bank to search without leaving the draft. Ordinary entries insert at the current cursor. Address / addressee entries populate the protected recipient block. Personal entries are private; shared entries are published by a workspace administrator.</Instruction>
              <Instruction number="8" title="Choose the drafting scope">Selected Issue context is used by default. Turn it off only when preparing a brief purpose-led draft that should not draw on the summary, communications or references.</Instruction>
              <Instruction number="9" title="Edit and format the body">The application protects the official heading, number, subject, recipient and signature from AI while keeping their values editable through Draft tools. Use undo, redo, bold, italic, underline, lists, tables and alignment in the body. Font, size, margins, line spacing, paragraph spacing and recipient indentation carry into Word export.</Instruction>
              <Instruction number="10" title="Review and save">Open Draft tools &gt; Review to find missing document details and unresolved placeholders. Verify every fact, citation, authority and approval, then use Save for routine changes. Use Save as separate version only when the current wording should be preserved independently. Record outgoing automatically preserves the issued copy.</Instruction>
            </ol>
          ...
          <HelpSection id="data" icon={Database} title="Data, cloud sync and backup" tone="rose">
            ...
            <GuideRows rows={[
              ['Cloud-synced now', 'Issue details, officers, communications, eReceipts, references, milestones, summary versions, notes and their revision history, saved drafts and the scoped Paragraph Bank sync through Neon for authorized users.'],
              ...
              ['Saved drafts', 'Generation remains temporary until Save is selected. Save updates the current draft; Save as separate version intentionally preserves another copy. The five newest draft records are retained and become available on another laptop after cloud sync.'],
              ['Draft tools', 'The Details tab can complete the communication number, date, subject, addressee, communication type and signatory at any point; Paragraph Bank inserts reusable wording or addresses; Review finds missing structured details and unresolved placeholders. The body editor provides undo, redo, emphasis, lists, tables and alignment, with document-wide font, size, margins, paragraph spacing and recipient indentation controls.'],
              ['Record outgoing', 'After saving the final draft, use Record outgoing to preserve the issued version and add it to the communication chronology with its communication type and authorised signatory.'],
              ...
              ['Working offline', 'If the browser reports that the network is unavailable, a banner confirms that changes remain on the device. When the network returns, the application attempts cloud synchronization and the cloud control shows whether it succeeded.'],
          ...
          <HelpSection id="api" icon={Cloud} title="Cloud AI" tone="slate" badge="Provider account">
            ...
            <GuideRows className="mt-4" rows={[
              ...
              ['API keys', 'Keys remain in protected Google Cloud Secret Manager and Cloud Run settings. They are never stored in browser settings, Neon user settings or VITE_ variables.'],
            ]} />
          </HelpSection>
        </div>
      </div>
