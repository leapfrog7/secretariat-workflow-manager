...
export function ToastProvider({ children }) {
  ...
  return (
    <ToastContext.Provider value={value}>
      {children}
      <div className="pointer-events-none fixed bottom-[calc(4.5rem+env(safe-area-inset-bottom))] right-3 z-[60] flex w-[calc(100%-1.5rem)] max-w-sm flex-col gap-2 sm:bottom-auto sm:top-4 sm:right-4" aria-live="polite" aria-atomic="false">
        {toasts.map((toast) => {
          const Icon = toast.type === 'error' ? XCircle : CheckCircle2;
          return (
            <div key={toast.id} role={toast.type === 'error' ? 'alert' : 'status'} className={`toast-enter pointer-events-auto relative overflow-hidden rounded-[var(--swm-radius-lg)] border bg-white p-3.5 pr-11 text-sm text-slate-800 shadow-[var(--swm-shadow-float)] ${toast.type === 'error' ? 'border-red-200' : 'border-emerald-200'}`}>
