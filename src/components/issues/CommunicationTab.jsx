            ...
            {communications.map((communication) => (
              <li key={communication.id} className="grid gap-3 px-4 py-4 sm:grid-cols-[130px_minmax(0,1fr)_auto]">
                ...
                <div className="min-w-0">
                  {communication.draftId && <div className="mb-2 flex flex-wrap gap-2"><span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-900">{communication.officialCommunicationType || 'Official communication'}</span><span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-semibold text-slate-600">Draft v{communication.draftVersion || '?'}</span>{communication.signatoryName && <span className="rounded-full bg-teal-50 px-2 py-0.5 text-xs font-semibold text-teal-800">Signed by {communication.signatoryName}</span>}</div>}
