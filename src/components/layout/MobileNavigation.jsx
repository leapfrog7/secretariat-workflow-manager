...
import { BookMarked, BookOpenCheck, ClipboardList, Ellipsis, FilePenLine, FilePlus2, FileText, House, LoaderCircle, Settings, UserRoundCog } from 'lucide-react';
...
export default function MobileNavigation() {
  ...
  return (
    <nav ref={containerRef} className="app-mobile-navigation fixed inset-x-0 bottom-0 z-40 border-t border-[var(--swm-border)] bg-white/95 pb-[env(safe-area-inset-bottom)] shadow-[0_-8px_28px_rgb(15_49_56_/_0.08)] backdrop-blur-xl" aria-label="Mobile navigation">
      {menuOpen && (
        <div role="menu" aria-label="More navigation" className="popover-enter absolute inset-x-3 bottom-[calc(100%+0.65rem)] z-50 origin-bottom overflow-hidden rounded-[var(--swm-radius-xl)] border border-[var(--swm-border)] bg-white p-2 shadow-[var(--swm-shadow-float)]">
          {secondaryItems.map((item, index) => {
            ...
            return (
              <Link
                ...
                className={`flex min-h-11 items-center gap-3 rounded-[var(--swm-radius-md)] px-3 text-sm font-semibold transition-colors ${isActive ? 'bg-teal-50 text-teal-900' : 'text-slate-700 hover:bg-slate-50'}`}
              >
